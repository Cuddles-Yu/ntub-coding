...
function searchStores($keyword, $userLat, $userLng)
{
    ...
    $sql = "SELECT DISTINCT s.id, s.name, s.preview_image, s.link, s.website, r.avg_ratings, r.total_reviews, l.city, l.details, s.tag, r.environment_rating, r.product_rating, r.service_rating, r.price_rating, l.latitude, l.longitude
            $haversinePart 
            FROM stores AS s
    ...
    // 若有經緯度條件，則加上範圍過濾
    if ($userLat && $userLng) {
        $sql .= " HAVING distance <= 1500";
        $sql .= " ORDER BY distance, r.avg_ratings DESC, r.total_reviews DESC";
    } else {
        $sql .= " ORDER BY r.avg_ratings DESC, r.total_reviews DESC";
    }
...
<div id="searchResults" class="store">
    <?php if (!empty($stores)) : ?>
        <?php foreach ($stores as $store) : ?>
            <div class="container-fluid store-body" onclick="redirectToDetailPage('<?php echo htmlspecialchars($store['id']); ?>')"> <!--整框商家 可點下進入詳細頁面-->
                <div class="row">
